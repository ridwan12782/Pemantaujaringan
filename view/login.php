      ...
      if ($ss === 'PemantauJaringan') {
        ...
        if (isset($_POST['login'])) {
            ...
            if ($user === $useradm && $pass === dec_rypt($passadm)) {
                $_SESSION["PemantauJaringan"] = $user;
                echo $urlLogin;
            } else {
...
<center>
  ...
  <span class="login-title">Login to Pemantau Jaringan</span>
</center> 
</div>
    <?= $error; ?>
    <div class="login-container">
      <form autocomplete="off" action="" method="post">
            ...
            <label for="_passw">Kata Sandi</label>
            <input class="form-control" type="password" name="pass" id="_passw" required="1">
            <div id="webview"></div>
            <button id="btn-login" class="bg-primary" type="submit" name="login" >Masuk</button>
            <!-- <?= $error; ?> -->
      </form>
    </div>
    <div class="mr-t-10 login-container pd-10 text-center text-dp">
      <span style="font-size:15px;font-weight:  bold; font-variant: small-caps;">Pemantau Jaringan</span>
    </div>
  </div>
  </div>
  </div>
