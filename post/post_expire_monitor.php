...
if(isset($_POST['sessname']) && isset($_SESSION["PemantauJaringan"])){
    ...
    $API->connect($iphost, $userhost, dec_rypt($passwdhost));
  	...
  	$get_expire_mon = $API->comm("/system/scheduler/print", array(
            "?name" => "PemantauJaringan-Expire-Monitor",));

    if($get_expire_mon[0]['name'] !== "PemantauJaringan-Expire-Monitor"){
      $expmon = $API->comm("/system/scheduler/add", array(
        ...
        "disabled" => "no",
        "comment" => "PemantauJaringan Expire Monitor",
        ));
